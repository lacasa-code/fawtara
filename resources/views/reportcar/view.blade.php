      ...
      <div class="nav_menu">
         <nav>
            <div class="nav toggle">
               <a id="menu_toggle"><i class="fa fa-bars"></i><span class="titleup">&nbsp; {{ trans('app.reportcar_List')}}</span></a>
            </div>
      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
         <div class="x_content">
            <ul class="nav nav-tabs bar_tabs tabconatent customTabWidth" role="tablist">
               @can('jobcard_view')
                  <li role="presentation" class=""><a href="{!! url('/reportcar/list')!!}"><span class="visible-xs" ></span><i class="fa fa-list fa-lg">&nbsp;</i>{{ trans('app.reportcar_List')}}</span></a></li>
               @endcan
   ...
   <div class="row">
            ...
            <form method="post" action="{!! url('/jobcard/store') !!}" class="addJobcardForm">
               ...
               <div class="col-md-12 col-xs-12 col-xs-12">
                  ...
                  <div class="col-md-5 col-xs-12 col-sm-12">
                     <div class="col-md-12 col-xs-12 col-sm-12">
                        <label class="control-label jobcardmargintop col-md-4 col-sm-12 col-xs-12">{{ trans('app.quotation_No')}} <label class="color-danger">*</label></label>
      ...
      <!-- Checking for Wash Bay Service Starting -->
         @if($washbay_data != null)
            <div class="col-md-12 col-xs-12 col-sm-12 panel-group">
               <div class="col-md-10 col-sm-8 col-xs-8">
                  <h2>{{ trans('app.Wash Bay Service Charge')}}</h2>
               </div>
            </div>
                        ...
                        <tr>
                           ...
                           <td>
                              @if($washbay_data->initiate_status == 0)
                                 <button type="button" class="btn btn-danger initiateProcessBtn" id="initiateProcessButton" url_init_process="{{ url('/jobcard/get_initial_process_status') }}">
                                    <span id="" class="initiateButtonSpin glyphicon "></span>
                                    {{ trans('app.Initiate Process')}}
                                 </button>
                              @elseif($washbay_data->initiate_status == 1 || $washbay_data->initiate_status == 2)
                                 <button type="button" class="btn btn-danger initiateProcessBtn" disabled="true">{{ trans('app.Initiate Process')}}</button>
                              @endif
                              ...
                              @if($washbay_data->initiate_status == 1)
                                 ...
                                 {{ trans('app.Complete Process')}}</button> &emsp;
                              @elseif($washbay_data->initiate_status == 0 || $washbay_data->initiate_status == 2)
                                 <button type="button" class="btn btn-success completeProcess" disabled="true" >
                                 <span id="" class="completeProcessSpin glyphicon "></span>
                                 {{ trans('app.Complete Process')}}</button> &emsp;
                              @endif

                              <input type="checkbox" name="notifyCustomer" class="text-right notifyCustomerCheckbox" id="" checked> {{ trans('app.Notify Customer')}}
                              <!-- <label class="control-label text-right" for="notifyCustomer">{{ trans('app.Notify Customer')}} <label class="text-danger"></label></label> -->
                           </td>
                        </tr>
                     </tbody>
                  </table>
...
$(document).ready(function()
{
   ...
   $('body').on('blur','.qtyt',function(){
      ...
      var url = $(this).attr('url');
      var msg5 = "{{ trans('app.Product Not Available')}}";
      var msg6 = "{{ trans('app.Current Stock :')}}";
   ...
   var emailMsg1 = "{{ trans('app.Email Notification')}}";
   var emailMsg2 = "{{ trans('app.Successfully Email Sent!')}}";
   ...
   $('.initiateProcessBtn').click(function(){
      ...
      $.ajax({
         type: 'GET',
         url: url,
         data : {serviceId:serviceId, notifyCustomerValue:notifyCustomerValue},
         success: function (response)
         {
            //alert("hi " + response['done'] +" " + response['notify'] +" " + response['isupdate'] +" " + response['status']);
            if (response == 1)
            {
               swal({
                  title: emailMsg1,
            ...
            $('.completeProcess').attr("url_complete_process", "{{ url('/jobcard/complete_process_status') }}");
         },
         error: function(e) {
            console.log(e);
         }
   ...
   $('.completeProcess').click(function(){
      ...
      //alert("hello " + url + "---- " + notifyCustomerValue + "---- " +serviceId);
