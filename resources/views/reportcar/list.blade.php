				...
				<div class="nav_menu">
					<nav>
				  		<div class="nav toggle">
							<a id="menu_toggle">
								<i class="fa fa-bars"></i><span class="titleup">&nbsp {{ trans('app.reportcar')}}</span>
							</a>
				  		</div>
						...
						<ul class="nav nav-tabs bar_tabs tabconatent" role="tablist">
							...
							<li role="presentation" class="active">
								<a href="{!! url('/jobcard/list')!!}">
									<span class="visible-xs"></span><i class="fa fa-list fa-lg">&nbsp;</i><b>{{ trans('app.reportcar_List')}}</b></span>
								</a>
							</li>
							@endcan
										...
										<tr>
											...
											@if(getUserRoleFromUserTable(Auth::User()->id) == 'admin' || getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee' || getUserRoleFromUserTable(Auth::User()->id) == 'branch_admin')
												@if(Gate::allows('jobcard_view') && Gate::allows('jobcard_edit') && Gate::allows('jobcard_add'))
													...
													@if($jobcard == 1)
														<a href="{{ url('quotation/list/modify/'.$servicess->id)}}" ><button type="button" class="btn btn-round btn-success" disabled>{{ trans('app.process_quotation')}}</button></a>
													@elseif($view_data == "Yes")
														<a href="{{ url('quotation/list/modify/'.$servicess->id)}}" ><button type="button" class="btn btn-round btn-success" disabled>{{ trans('app.process_quotation')}}</button></a>
													@else
														<a href="{{ url('quotation/list/modify/'.$servicess->id)}}" ><button type="button" class="btn btn-round btn-success" >{{ trans('app.process_quotation')}}</button></a>
													@endif


												  	@endcan
												@elseif(getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee'|| getUserRoleFromUserTable(Auth::User()->id) == 'reportcar')
													@can('jobcard_view')
													<?php
													$view_data = getInvoiceStatus($servicess->job_no);
