											...
											@if(getUserRoleFromUserTable(Auth::User()->id) == 'admin' || getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee' || getUserRoleFromUserTable(Auth::User()->id) == 'branch_admin')
												@if(Gate::allows('jobcard_view') && Gate::allows('jobcard_edit') && Gate::allows('jobcard_add'))
													...
													if($view_data == "No")
													{
														...
														{
														?>
															<?php /* <a href="{{ url('jobcard/list/add_invoice/'.$servicess->id) }}"><button type="button" class="btn btn-round btn-info">{{ trans('app.Create Invoice')}} </button></a>	*/ ?>

															<a href="{{ url('/invoice/manual/invoice') }}"><button type="button" class="btn btn-round btn-info">{{ trans('app.Create Invoice')}} </button></a>			
														<?php 
														}
														elseif($servicess->done_status != '1'  )
														{		
														?>
															<?php /*<a href="{{ url('jobcard/list/add_invoice/'.$servicess->id) }}"><button type="button" class="btn btn-round btn-info" disabled>{{ trans('app.Create Invoice')}} </button></a>  */ ?>
