            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_content">
                    <ul class="nav nav-tabs bar_tabs tabconatent" role="tablist">
                        ...
                        <li role="presentation" class="active">
                            <a href="{!! url('/engineer/list')!!}">
                                <span class="visible-xs"></span><i class="fa fa-list fa-lg">&nbsp;</i><b>{{ trans('app.engineer_List') }}</b>
                            </a>
                        </li>
                        @endcan
                        @can('engineer_add')
                        <li role="presentation" class=" setTabAddSupportStaffOnSmallDevice">
                            <a href="{!! url('/engineer/add')!!}">
                                <span class="visible-xs"></span><i class="fa fa-plus-circle fa-lg">&nbsp;</i> {{ trans('app.engineer_add') }}
                            </a>
                        </li>
                        @endcan
                    </ul>
                ...
                <div class="x_panel bgr">
                    <table id="datatable" class="table table-striped jambo_table" style="margin-top:20px;">
                      <thead>
                          <tr>
                            ...
                            @elseif(getUserRoleFromUserTable(Auth::User()->id) == 'Customer')
                              @if(Gate::allows('engineer_add') || Gate::allows('engineer_edit') || Gate::allows('engineer_delete'))
                                <th>{{ trans('app.Action')}}</th>
                              @endif
                            @endif
                          </tr>
                      ...
                      <tbody>
                          <?php $i=1;?>
                              @foreach($engineer as $supportstaffs)
                                <tr>
                                    <td>{{ $i }}</td>
                                    <td>
                                        <img src="{{ url('public/engineer/'.$supportstaffs->image) }}"  width="50px" height="50px" class="img-circle" >
                                    </td>
                                  ...
                                  @if(getUserRoleFromUserTable(Auth::User()->id) != 'Customer')
                                    ...
                                    @if(getUserRoleFromUserTable(Auth::User()->id) == 'admin' ||  getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee' || getUserRoleFromUserTable(Auth::User()->id) == 'branch_admin')
                                      @can('engineer_view')
                                        <a href="{!! url('/engineer/list/'.$supportstaffs->id) !!}"><button type="button" class="btn btn-round btn-info">{{ trans('app.View')}}</button></a>
                                      @endcan
                                      @can('engineer_edit')
                                        <a href="{!! url ('/engineer/list/edit/'.$supportstaffs->id) !!}"><button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>
                                      @endcan
                                      @can('engineer_delete')
                                        <a url="{!! url('/engineer/list/delete/'.$supportstaffs->id)!!}" class="sa-warning"><button type="button" class="btn btn-round btn-danger">{{ trans('app.Delete')}}</button></a>
                                      @endcan
