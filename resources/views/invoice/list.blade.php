					...
					<div class="col-md-12 col-sm-12 col-xs-12" >
            			<div class="x_content">
							<ul class="nav nav-tabs bar_tabs" role="tablist">
								...
								@endcan
								@can('invoice_add')
									<li role="presentation" class=""><a href="{!! url('/invoice/addInvoice')!!}"><span class="visible-xs"></span> <i class="fa fa-plus-circle fa-lg">&nbsp;</i>
									AddInvoice </a></li>
								@endcan
								@can('invoice_add')
								...
								<tr class="texr-left">
									...
									@if(getUserRoleFromUserTable(Auth::User()->id) == 'admin' || getUserRoleFromUserTable(Auth::User()->id) == 'supportstaff' || getUserRoleFromUserTable(Auth::User()->id) == 'accountant' || getUserRoleFromUserTable(Auth::User()->id) == 'employee' || getUserRoleFromUserTable(Auth::User()->id) == 'branch_admin')
										@if($invoices->type != 2)
											...
											<a href="{{ route('showInvoice2', ['id' => $invoices->id]) }}" type="button" class="btn btn-primary btn-round"> Show </a>
										...
										@else
											...
											<a href="{{ route('showInvoice', ['id' => $invoices->id]) }}" type="button" class="btn btn-primary btn-round"> Show </a>
...
$(document).ready(function() 
{
	...
	/*Delete invoice*/
 	...
 	$('body').on('click', '.payWarning', function() {
 		...
 		var msg5 = "{{ trans('app.Stripe Payment Failed')}}";
	    var msg6 = "{{ trans('app.You can not pay more than 999999.99 in a single transaction using card!')}}";
