...
?>
    ...
    <?php
class ReportcarControler extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        else
        {
            if(!empty($request->free))
            {
                $services = Service::orderBy('service_date','asc')->where([['job_no','like','J%'],['service_type','=','free'],['done_status','=',1],['soft_delete','=',0], ['branch_id',$adminCurrentBranch->branch_id]])->whereNotIn('quotation_modify_status',[1])->get();
            }
            elseif(!empty($request->paid))
            {
                $services = Service::orderBy('service_date','asc')->where([['job_no','like','J%'],['service_type','=','paid'],['done_status','=',1],['soft_delete','=',0], ['branch_id',$adminCurrentBranch->branch_id]])->whereNotIn('quotation_modify_status',[1])->get();
            }
            elseif(!empty($request->repeatjob))
            {
                $services = Service::orderBy('service_date','asc')->where([['job_no','like','J%'],['service_category','=','repeat job'],['done_status','=',1],['soft_delete','=',0], ['branch_id',$adminCurrentBranch->branch_id]])->whereNotIn('quotation_modify_status',[1])->get();
            }
            else
            {
        ...
        return view('reportcar.list',compact('services','available'));
    }
    ...
    public function getrecord(Request $request)
    {
        ...
        $all_sql = DB::select("SELECT * FROM `tbl_services`
				...
				INNER JOIN tbl_reportcard_details ON tbl_services.id = tbl_reportcard_details.service_id
    ...
    public function store(Request $request)
    {
        ...
        $out_date = DB::update("update tbl_reportcard_details set out_date='$odate' where service_id=$service_id");
        ...
        $tblcountjob=DB::table('tbl_reportcard_details')->where('jocard_no','=',$job_no)->count();
        if($tblcountjob == 0)
        {
        ...
        {
            DB::table('tbl_reportcard_details')
                ->where('service_id', $service_id)
                ->update(['out_date' => $odate, 'kms_run' => $kms]);
        }

        DB::update("update `tbl_services` set done_status=1 where id=$service_id");
        DB::update("update `tbl_reportcard_details` set done_status=1 where service_id=$service_id");
    ...
    public function view($id)
    {
        ...
        $service_data = ReportcardDetail::latest()->first();

        if(!empty($v_id)){
            $vehicale = Vehicle::where('id','=',$v_id)->first();
        }

        $job = ReportcardDetail::where('service_id','=',$id)->first();
        ...
        return view('reportcar.view',compact('viewid','services','tbl_observation_points','tbl_observation_service','tbl_service_observation_points','vehicale','sales','product','s_id','job','pros','pros2','tbl_checkout_categories','first','vehicalemodel','tbl_points','s_date','color','service_data','tax','logo','obser_id','data','fetch_mot_test_status','employees', 'washbay_data'));
    }
    ...
    public function get_initial_process_status(Request $request)
    {
            ...
            if ($notify_customer == 'true')
            {
                ...
                if($emailformats->is_send == 0)
                {
                    ...
                    else
                    {
                        ...
                        //$data = mail($emails,$mail_sub,$email_content,$headers);
                    }
        ...
        //return response()->json(['done' => $done, 'notify' => $notify_customer, 'isupdate' => $is_update, 'status' => 1]);
        return $done;
    }
    ...
    public function complete_process_status(Request $request)
    {
        ...
        //return response()->json(['done' => $done, 'notify' => $notify_customer, 'isupdate' => $is_update, 'status' => 2]);
        return $done;
    }
