...
class BranchAdminController extends Controller
{
	...
	public function index()
	{
		...
		{
			// $branchadmins = User::where([['role','=','branch_admin'],['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','DESC')->get();
		...
		{
			$branchadmins = User::where([['role','=','branch_admin'],['soft_delete',0], ['branch_id',$currentUser->branch_id]])->orderBy('id','DESC')->get();
		}
	...
	public function storeBranchAdmin(StoreBranchAdminAddEditRequest $request)
	{
        ...
        $branchadmin->employeeid =$employeeid;
        $branchadmin->join_date = $join_date;
		...
		return redirect('/branchadmin/list')->with('message','Successfully Submitted');
	}
    ...
    public function branchAdminUpdate($id, StoreBranchAdminAddEditRequest $request)
	{
		...
		return redirect('/branchadmin/list')->with('message','Successfully Updated');
	}
	...
	{
		//$branchadmin = DB::table('users')->where('id','=',$id)->delete();
		$branchadmin = User::where('id','=',$id)->update(['soft_delete' => 1]);

		return redirect('/branchadmin/list')->with('message','Successfully Deleted');
	}
}
