...
class Purchasecontroller extends Controller
{
	...
	{		
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			$product = Product::where([['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();
		...
		{
			$product = Product::where([['soft_delete',0], ['branch_id',$currentUser->branch_id]])->get();
	...
	public function getproductname(Request $request)
	{
		...
		//dd($product->toArray(), $prd_type_id_array, $first_product, $Select_product);
