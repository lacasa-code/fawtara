...
class QuotationController extends Controller
{
    ...
    public function list()
    {
		...
		if (!isAdmin(Auth::User()->role_id))
		{
			...
			{
				$service = DB::table('tbl_services')->where([['job_no','like','J%'],['assign_to','=',Auth::User()->id], ['is_quotation', '=', 1], ['branch_id',$currentUser->branch_id]])->orderBy('id','DESC')->get()->toArray();
			}
			elseif (getUsersRole(Auth::user()->role_id) == 'Support Staff' || getUsersRole(Auth::user()->role_id) == 'Accountant' || getUsersRole(Auth::user()->role_id) == 'Branch Admin') {
				...
				$service = DB::table('tbl_services')->where([['job_no','like','J%'],['is_quotation', '=', 1], ['branch_id',$currentUser->branch_id]])->orderBy('id','DESC')->get()->toArray();
			}
		}
		else {
			$service=DB::table('tbl_services')->where([['job_no','like','J%'],['is_quotation', '=', 1], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','DESC')->get()->toArray();
		}
	...
	public function quotationModify($id)
	{
        ...
        if (isAdmin(Auth::User()->role_id)){
            ...
            $branchDatas = Branch::where('id', $adminCurrentBranch->branch_id)->get();
            $employee = DB::table('users')->where([['role','supportstaff'],['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get()->toArray();
        }
        elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
            $branchDatas = Branch::get();
            $employee = DB::table('users')->where([['role','supportstaff'],['soft_delete',0]])->get()->toArray();
        }
        else{
            $branchDatas = Branch::where('id', $currentUser->branch_id)->get();
            $employee = DB::table('users')->where([['role','supportstaff'],['soft_delete',0], ['branch_id',$currentUser->branch_id]])->get()->toArray();
        }
		$viewid = $id;
		$first = $color = null;
		...
		return view('quotation.quotationmodify',compact('viewid','employee','services','tbl_observation_points','tbl_observation_service','tbl_service_observation_points','vehicale','sales','product','s_id','job','pros','pros2','tbl_checkout_categories','first','vehicalemodel','tbl_points','s_date','color','service_data','tax','logo','obser_id','data','fetch_mot_test_status','washbayPrice'));
	}
	...
	public function add_modify_quotation(Request $request)
	{
		...
		$Assignestaff= $request->assignestaff;

        #dd($request->assignestaff);
        #exit();
		if(getDateFormat() == 'm-d-Y'){
		...
		if ($finalSubmit != null) {
            ...
            Service::where('id', '=', $service_id)->update(['is_quotation' => 1]);
            Service::where('id', '=', $service_id)->update(['assign_staff' =>$Assignestaff]);
	...
	//Quotation edit (Quotation service add first step edit)
   	public function quotationEdit($id)
   	{
		...
		else
		{
   		...
   		return view('/quotation/edit',compact('service','vehical','employee','customer','regi_no','tbl_custom_fields', 'tax','inspection_points_library_data','mot_inspections_answers', 'washbayPrice', 'branchDatas'));
   	}
