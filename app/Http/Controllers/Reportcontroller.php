...
{	
    ...
    public function sales()
	{
		...
		//dd($Sales, $Select_customer, $Select_salesman, $salesreport);
    ...
    public function service(Request $request)
	{
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			$servicereport = Service::where([['done_status','=',1], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','desc')->get();
		}
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$servicereport = Service::where('done_status','=',1)->orderBy('id','desc')->get();
		}
		else{
			$servicereport = Service::where([['done_status','=',1], ['branch_id',$currentUser->branch_id]])->orderBy('id','desc')->get();
		}
	...
	{	
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			...
			{			
				...
				$servicereport = Service::where('done_status','=',1)->where([['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','desc')->get();
			...
			elseif($s_date == "" && $e_date == "" && $all_service =='free'){
				...
				$servicereport = Service::where([['done_status','=',1],['service_type','=',$all_service]])->where([['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','desc')->get();
	...
	public function product()
	{
		...
		if (isAdmin(Auth::User()->role_id)) 
		{			
			$productname = Product::where([['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();			
		...
		->JOIN('tbl_purchase_history_records','tbl_purchase_history_records.purchase_id','=','tbl_purchases.id')
		->JOIN('tbl_products','tbl_products.id','=','tbl_purchase_history_records.product_id')
		//->orderBy('tbl_purchase_history_records.purchase_id','asc')
		->GROUPBY('tbl_purchase_history_records.product_id')
		...
		//dd($productreport, $productname, $Select_product, $product);
	...
	{	
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			...
			else if($all_product !='all' && $all_item =='item')
			{
				...
				$productname = Product::where('tbl_products.product_type_id','=',$all_product)->where([['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();
			}
			else if($all_product == 'all' && $all_item != 'item')
			{
	...
	{	
		...
		//dd($totalstock1, $cellstock1, $Select_product, $productname, $productreport, $s_date, $e_date);
	...
	public function uses_product(Request $request)
	{
		...
		if (isAdmin(Auth::User()->role_id))
		{
			...
			elseif($s_date != "" && $e_date != "" && $all_product != 'all' && $all_item != 'item')
			{
				...
				$productname = Product::where([['product_type_id','=',$all_product], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();
			}
			elseif($s_date != "" && $e_date != "" && $all_product != 'all' && $all_item == 'item')
			{
				$productreport = DB::table('tbl_purchase_history_records')
		...
		$Select_product = DB::table('tbl_product_types')->get()->toArray();
		//dd($Select_product, $productname, $productreport, $s_date, $e_date);
	...
	public function servicebyemployee()
	{
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			$servicereport = Service::where([['done_status','=',1], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','desc')->get();

			$service = DB::select("SELECT count(*) as counts, RIGHT(YEAR(`service_date`),4) as date FROM tbl_services where done_status=1 and branch_id= '" .$adminCurrentBranch->branch_id. "'  group by YEAR(`service_date`) ORDER BY service_date ASC");

			$Select_employee = DB::table('users')->where([['role','=','employee'], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get()->toArray();
		}
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') 
		{
			$servicereport = Service::where('done_status','=',1)->orderBy('id','desc')->get();
		...
		//dd($servicereport, $service, $Select_employee);
	...
	{	
		...
		if (isAdmin(Auth::User()->role_id)) 
		{
			if($s_date == "" && $e_date == "" && $all_employee == 'all')
			{
				...
				$service = DB::select("SELECT count(*) as counts, RIGHT(YEAR(`service_date`),4) as date FROM tbl_services where (assign_to='$all_employee') and (done_status=1) and '" .$adminCurrentBranch->branch_id. "' group by YEAR(`service_date`) ORDER BY date ASC");
			...
			elseif($s_date != "" && $e_date != "" && $all_employee == 'all')
			{
				...
				$service = DB::select("select count(*) as counts, CONCAT(	MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where  (service_date BETWEEN  '$s_date' AND '$e_date') and (done_status=1) and '" .$adminCurrentBranch->branch_id. "' GROUP BY MONTH(service_date), YEAR(service_date)");
			
				$Select_employee = User::where([['role','=','employee'], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();

				$servicereport = Service::where([['done_status','=',1], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->whereBetween('service_date', array($s_date, $e_date))->orderBy('id','desc')->get();
			}
			else if($s_date == "" && $e_date == "" && $all_employee != 'all')
			{
				...
				$service = DB::select("SELECT count(*) as counts, RIGHT(YEAR(`service_date`),4) as date FROM tbl_services where (assign_to='$all_employee') and (done_status=1) and '" .$adminCurrentBranch->branch_id. "' group by YEAR(`service_date`) ORDER BY service_date ASC");			

				$Select_employee = User::where([['role','=','employee'], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get();

				$servicereport = Service::where([['assign_to','=',$all_employee],['done_status','=',1], ['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','desc')->get();
			}
			else if($s_date != "" && $e_date != "" && $all_employee != 'all')
			{
				$title_report = 'All Service';
				...
				$service = DB::select("select count(*) as counts, CONCAT(MONTHNAME(`service_date`),'-',RIGHT(YEAR(`service_date`),4)) as date FROM tbl_services where (service_date BETWEEN  '$s_date' AND '$e_date') and (done_status=1) and assign_to='$all_employee' and '" .$adminCurrentBranch->branch_id. "' GROUP BY MONTH(service_date), YEAR(service_date)");
