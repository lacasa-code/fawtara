...
class Engineerstaffcontroller extends Controller
{
	...
	public function index()
	{
		...
		{
			$engineer = User::where([['role','=','engineer'],['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->orderBy('id','DESC')->get();
		}
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$engineer = User::where([['role','=','engineer'],['soft_delete',0]])->orderBy('id','DESC')->get();
		}
		else
		{
			$engineer = User::where([['role','=','engineer'],['soft_delete',0], ['branch_id',$currentUser->branch_id]])->orderBy('id','DESC')->get();
		}

		return view('engineer.list',compact('engineer'));
	}
