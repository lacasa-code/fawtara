...
class homecontroller extends Controller
{
    ...
    public function dashboard()
    {
    	...
    	$Customer = null;
    	$Customer = $Supplier = $employee = $product = $sales = $service = $engineer =null;
		$data = "";
		$one_day = "";
		$two_day = "";
		$more = "";
      ...
      #  dd(getUsersRole(Auth::user()->role_id));
        #exit();
		...
		elseif (getUsersRole(Auth::user()->role_id) == 'Support Staff' || getUsersRole(Auth::user()->role_id) == 'Accountant' || getUsersRole(Auth::user()->role_id) == 'Employee' || getUsersRole(Auth::user()->role_id) == 'Branch Admin' || getUsersRole(Auth::user()->role_id) == 'Engineer')
		{
		...
		if (!isAdmin(Auth::User()->role_id))
		{
			...
			elseif (getUsersRole(Auth::user()->role_id) == 'Employee')
			{
                ...
                $engineer = User::where([['role','=','engineer'],['soft_delete','=',0], ['branch_id',$currentUser->branch_id]])->count();
			...
			elseif (getUsersRole(Auth::user()->role_id) == 'Support Staff' || getUsersRole(Auth::user()->role_id) == 'Engineer' ||getUsersRole(Auth::user()->role_id) == 'Accountant' || getUsersRole(Auth::user()->role_id) == 'Branch Admin')
			{

				$employee = User::where([['role','=','employee'],['soft_delete','=',0], ['branch_id',$currentUser->branch_id]])->count();
				$Customer = User::where([['role','=','Customer'],['soft_delete','=',0]])->count();
                $engineer = User::where([['role','=','engineer'],['soft_delete','=',0]])->count();
				$Supplier = User::where([['role','=','Supplier'],['soft_delete','=',0]])->count();
				...
				$service = Service::where([['job_no','like','J%'],['soft_delete','=',0], ['branch_id',$currentUser->branch_id]])->count();
		...
		else
		{
			...
			$service = Service::where([['job_no','like','J%'],['soft_delete','=',0], ['branch_id',$adminCurrentBranch->branch_id]])->count();
       ...
       # var_dump($vehical);
        #exit();
      return view('dashboard.dashboard',compact('employee','engineer','Customer','Supplier','product','sales','service','Customere','sale','sale1','sale2','dates','data','vehical','performance','serviceevent','one_day','two_day','more','nowmonth','openinghours','holiday','upcomingservice', 'have_supportstaff', 'have_vehicle', 'have_product', 'have_purchase', 'have_observationCount'));
