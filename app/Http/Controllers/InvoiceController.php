...
{	
	...
	{
		$invoice =  Invoice::findOrFail($id);
		return view('invoice.ShowInvoice2',compact('invoice'));
	}
	...
	{		
		...
		return view('invoice.add',compact('code','tax','tbl_sales','codepay','total_rto','tbl_payments','tbl_custom_fields', 'branchDatas')); 
	}
	...
	{	
		...
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$getJobcardNoFromServiceTbl = DB::table('tbl_services')->where([['customer_id','=', $cus_id],['done_status','=',1],['job_no','like','J%']])->where('soft_delete', 0)->get()->toArray();
			$invoiceTblJobcardNo = DB::table('tbl_invoices')->where([['customer_id','=', $cus_id],['job_card','like','J%'],['type', '=', 0]])->where('soft_delete', 0)->get()->toArray();
		}
		else{
			$getJobcardNoFromServiceTbl = DB::table('tbl_services')->where([['customer_id','=', $cus_id],['done_status','=',1],['job_no','like','J%'], ['branch_id',$currentUser->branch_id]])->where('soft_delete', 0)->get()->toArray();
			$invoiceTblJobcardNo = DB::table('tbl_invoices')->where([['customer_id','=', $cus_id],['job_card','like','J%'],['type', '=', 0], ['branch_id',$currentUser->branch_id]])->where('soft_delete', 0)->get()->toArray();
		}
	...
	public function store(Request $request)
	{
		...
		if($type != 2)
		{
			...
			if(!empty($type == 0))
			{
				...
				$pdf = PDF::loadView('invoice.serviceinvoicepdf',compact('serviceid','tbl_services','sales','logo','job','s_date','vehical','customer','service_pro','service_pro2','tbl_service_observation_points','service_tax','service_taxes','discount', 'washbay_data'));
				$str = "1234567890";
				$str1 = str_shuffle($str);
	...
	public function payupdate(Request $request, $id)
	{
		...
		$adminCurrentBranch = BranchSetting::where('id','=',1)->first();		
		if (isAdmin(Auth::User()->role_id)){			
			$branchDatas = Branch::where('id', $adminCurrentBranch->branch_id)->first();
		}
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$branchDatas = "";
		}
	...
	{	
		...
		return view('invoice/edit',compact('invoice_edit','customer','tax','dueamount','tbl_payments','tbl_custom_fields', 'branchDatas'));
	}
	...
	public function servicepdf($id)
	{
		...
		$pdf = PDF::loadView('invoice.serviceinvoicepdf',compact('serviceid','tbl_services','sales','logo','job','s_date','vehical','customer','service_pro','service_pro2','tbl_service_observation_points','service_tax','service_taxes','discount','tbl_custom_fields_invoice','tbl_custom_fields_service','tbl_custom_fields_customers','washbay_data'));
