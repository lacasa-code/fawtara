...
class ServicesControler extends Controller
{
    ...
    public function servicelist()
    {
		...
		if (!isAdmin(Auth::User()->role_id))
		{
			...
			elseif (getUsersRole(Auth::user()->role_id) == 'Support Staff' || getUsersRole(Auth::user()->role_id) == 'Accountant' || getUsersRole(Auth::user()->role_id) == 'Branch Admin') {
				...
				$service = Service::where([['job_no','like','J%'],['soft_delete','=',0],['is_quotation', '=', 0], ['branch_id',$currentUser->branch_id]])->orderBy('id','DESC')->get();
			}
		}
		else
		{
	...
	public function vehicleadd(Request $request)
	{
        ...
        if(!empty($request->License))
        {
            ...
            $vehical->license = $filename;
        }
        else
        {
            $vehical->license = 'RHPKP1.jpg';
        }
		$vehical->save();
        #########
        ...
        #$coloress = $request->color;
       # print($coloress);
        #foreach ($coloress as $colore)
        #{
            $colorse =$request->color;
    ...
    public function get_vehicle_name2(Request $request)
    {
        //$cus_id = Input::get('cus_id');
        $cus_id = $request->cus_id;
        #print_r($cus_id);
	...
	public function store(Request $request)
    {
    	...
    	$job = getJobcardNumber($request->jobno);
		...
		if(!empty($custom))
		{
      	...
      	//$washbay_price = DB::table('washbays')->where([['jobcard_no', '=', $job], ['customer_id', '=', $Customername]])->first();
        ...
        if ( Auth::user()->role=='employee' or Auth::user()->role=='admin' )
        {
            ...
            #return view('/service/report_form', compact('idid','service_data', 'vehical', 'tbl_checkout_categories', 'sale_date', 'color', 'obs_point', 'free_coupan', 'logo', 'inspection_points_library_data', 'washbay_price'));
            return view('/service/report_form',compact('service_data','vehical','tbl_checkout_categories','sale_date','color','obs_point','free_coupan','logo','inspection_points_library_data', 'washbay_price'));
        }else{
            #var_dump(Auth::user()->role);
            #exit();
            #return view('service/list');
            return redirect('/service/list')->with('message','Successfully Submitted');
        }
	  	#return view('/service/jobcard_form',compact('service_data','vehical','tbl_checkout_categories','sale_date','color','obs_point','free_coupan','logo','inspection_points_library_data', 'washbay_price'));
    }
#################################################################################
//add_jobcard store
    public function add_reportcard(Request $request)
    {
        $job_no = getJobcardNumber($request->job_no);
        $service_id = $request->service_id;
        $cus_id = $request->cust_id;
        $vehi_id = $request->vehi_id;
        $kms = $request->kms;
        ...
        $tbl_jobcard_details = new ReportcardDetail;
        $tbl_jobcard_details->customer_id = $cus_id;
        $tbl_jobcard_details->vehicle_id = $vehi_id;
        $tbl_jobcard_details->service_id = $service_id;
        $tbl_jobcard_details->jocard_no = $job_no;
        ...
        $tbl_jobcard_details->out_date = $out_dat;
        $tbl_jobcard_details->kms_run = $kms;
        $tbl_jobcard_details->fuel = $fuel;
        $tbl_jobcard_details->fltp=$fltp;
        $tbl_jobcard_details->frtp= $frtp;
        $tbl_jobcard_details->rltp=$rltp;
        $tbl_jobcard_details->rrtp=$rrtp;
        ##########################################
        $image = $request->image;
        ...
        if(!empty($image))
        {
            ...
            foreach($files as $file)
            {
                $filename = $file->getClientOriginalName();
                #print_r($filename);
                $file->move(public_path().'/report/', $file->getClientOriginalName());
                $images = new tbl_reportcard_images;
                $images->vehicle_id = $service_id;
                $images->image = $filename;
                $images->save();
            }
        }
        ...
        return redirect('reportcar/list')->with('message','Successfully Submitted');
    }
...
#################################################################################
	...
	//service edit
   	public function serviceedit($id)
   	{
		...
		{
			$branchDatas = Branch::where('id', $adminCurrentBranch->branch_id)->get();
			$employee=DB::table('users')->where([['role','engineer'],['soft_delete',0], ['branch_id',$adminCurrentBranch->branch_id]])->get()->toArray();
		}
		elseif (getUsersRole(Auth::user()->role_id) == 'Customer') {
			$branchDatas = Branch::get();
			$employee=DB::table('users')->where([['role','engineer'],['soft_delete',0]])->get()->toArray();
		}
		else
		{
			$branchDatas = Branch::where('id', $currentUser->branch_id)->get();
			$employee=DB::table('users')->where([['role','engineer'],['soft_delete',0], ['branch_id',$currentUser->branch_id]])->get()->toArray();
		}

   		return view('/service/edit',compact('service','vehical','employee','customer','regi_no','tbl_custom_fields', 'washbayPrice', 'branchDatas'));
   }
//service update
    public function serviceupdate(Request $request, $id)
    {
        ...
        if(!empty($sale_date))
        {
            ...
            $col = DB::table('tbl_vehicle_colors')->where('vehicle_id','=',$veh_id)->first();
            if(empty($col ->color))
            {
                $color='';
            }else{
   ...
   //service update
   public function serviceupdate2(Request $request, $id)
   {
